...
if not logger.handlers:
    ...
    formatter = logging.Formatter('[AI_MODEL] %(levelname)s: %(message)s')
...
class PerformanceConfig:
    ...
    batch_size: int = 32
    cache_size: int = 1000
    async_timeout: float = 300.0  # 5 минут
...
class ModelNotTrainedError(ModelError):
    """Исключение когда модель не обучена"""
    pass

class InsufficientDataError(ModelError):
    """Исключение при недостатке данных"""
    pass
...
class RFModel:
  ...
  def __init__(self, lottery_config: dict, n_estimators=150, random_state=42, min_samples_leaf=5, max_depth=20):
    """
    ...
    if not lottery_config:
      raise ValueError("RFModel требует объект конфигурации.")
    ...
    self.models_f1 = [
      RandomForestClassifier(**common_params)
    ...
    # Сохраняем параметры для восстановления размерности признаков
    self._feature_vector_length = 0

  def _count_consecutive(self, numbers: List[int]) -> int:
    """Подсчитывает максимальную длину последовательных чисел"""
    if len(numbers) < 2:
      return 0
    sorted_nums = sorted(set(numbers))  # Убираем дубликаты
    max_consecutive = 1
    current_streak = 1
  ...
  def _create_feature_vector(
      ...
      current_f1: List[int],
      current_f2: List[int],
      history_data: List[Tuple[List[int], List[int]]],
      df_history: Optional[pd.DataFrame] = None
  ) -> np.ndarray:
    """
    Единая функция создания вектора признаков для обучения и предсказания.
    Гарантирует идентичность признаков в обоих случаях.
    """
    features = []

    # 1. Числа из последних тиражей (отсортированные)
    lookback_draws = min(5, len(history_data))
    for j in range(lookback_draws):
      prev_f1, prev_f2 = history_data[j]
      features.extend(sorted(prev_f1) + sorted(prev_f2))

    # Дополняем нулями если не хватает истории
    total_numbers_per_draw = self.field1_size + self.field2_size
    while len(features) < lookback_draws * total_numbers_per_draw:
      features.append(0)

    # 2. Частоты каждого числа за последние 20 тиражей
    freq_window = min(20, len(history_data))
    recent_draws_f1 = []
    recent_draws_f2 = []

    for i in range(freq_window):
      f1, f2 = history_data[i]
      recent_draws_f1.extend(f1)
      recent_draws_f2.extend(f2)
    ...
    last_appearance_f1 = {num: -1 for num in range(1, self.field1_max + 1)}
    last_appearance_f2 = {num: -1 for num in range(1, self.field2_max + 1)}

    # Текущий тираж - расстояние 0
    for num in current_f1:
      if num in last_appearance_f1:
        last_appearance_f1[num] = 0
    for num in current_f2:
      if num in last_appearance_f2:
        last_appearance_f2[num] = 0

    # Ищем последнее появление в истории
    for k in range(len(history_data)):
      f1, f2 = history_data[k]
      for num in f1:
    ...
    stats_window = min(3, len(history_data))
    for j in range(stats_window):
      if j < len(history_data):
        prev_f1, prev_f2 = history_data[j]
      else:
        prev_f1, prev_f2 = [0] * self.field1_size, [0] * self.field2_size
      ...
      # Размах (макс - мин)
      features.append(max(prev_f1) - min(prev_f1) if prev_f1 else 0)
      features.append(max(prev_f2) - min(prev_f2) if prev_f2 else 0)

      # Дополнительные статистики для больших полей
      if self.field1_size >= 5:
        features.append(np.median(prev_f1) if prev_f1 else 0)
        features.append(len(set(prev_f1) & set(range(1, self.field1_max // 2 + 1))))
      if self.field2_size >= 3:
        features.append(np.median(prev_f2) if prev_f2 else 0)
        features.append(len(set(prev_f2) & set(range(1, self.field2_max // 2 + 1))))

    # 5. День недели (7 признаков) - используем текущее время
    try:
    ...
    if self.field1_max > 20:
      ...
      consecutive_count = self._count_consecutive(current_f1)
      features.append(consecutive_count)

    # Сохраняем длину вектора признаков для последующего использования
    self._feature_vector_length = len(features)

    return np.array(features, dtype=np.float32)

  def _prepare_rf_data(self, df_history: pd.DataFrame) -> Tuple[
    Optional[np.ndarray], Optional[List[np.ndarray]], Optional[List[np.ndarray]]]:
    """
    Подготовка данных для Random Forest обучения.
    """
    ...
    for i in range(lookback_draws, len(df_copy)):
      ...
      history_data = []
      for j in range(1, min(lookback_draws + 1, i + 1)):
        hist_f1 = df_copy.iloc[i - j]['Числа_Поле1_list']
        hist_f2 = df_copy.iloc[i - j]['Числа_Поле2_list']
        history_data.append((hist_f1, hist_f2))

      # Текущие числа для признаков
      current_f1 = df_copy.iloc[i - 1]['Числа_Поле1_list']
      current_f2 = df_copy.iloc[i - 1]['Числа_Поле2_list']

      # Создаем вектор признаков
      try:
        features = self._create_feature_vector(current_f1, current_f2, history_data, df_copy.iloc[:i])
        X_list.append(features)
      except Exception as e:
        print(f"AI Model (RF): Ошибка создания признаков для строки {i}: {e}")
        continue
  ...
  async def train_async(self, df_history: pd.DataFrame):
    """
    Асинхронная обертка для универсального обучения Random Forest моделей.
    """
    loop = asyncio.get_event_loop()
    # Используем ThreadPoolExecutor для CPU-интенсивной задачи
    with concurrent.futures.ThreadPoolExecutor() as executor:
      result = await loop.run_in_executor(
        executor,
        partial(self._train_sync_wrapper, df_history)
      )
    return result

  def _train_sync_wrapper(self, df_history):
    """Синхронная обертка для вызова основного метода обучения."""
    self.train(df_history)
    return self.is_trained

  async def predict_next_combination_async(self, last_draw_numbers_f1: List[int], last_draw_numbers_f2: List[int],
                                          df_history: Optional[pd.DataFrame] = None) -> Tuple[Optional[List[int]], Optional[List[int]]]:
    """
    Асинхронное универсальное предсказание следующей комбинации.
    """
    try:
      loop = asyncio.get_event_loop()
      with concurrent.futures.ThreadPoolExecutor(max_workers=PERF_CONFIG.max_workers) as executor:
        result = await asyncio.wait_for(
          loop.run_in_executor(
            executor,
            partial(self.predict_next_combination, last_draw_numbers_f1, last_draw_numbers_f2, df_history)
          ),
          timeout=PERF_CONFIG.async_timeout
        )
      return result
    except asyncio.TimeoutError:
      logger.error(f"RF prediction timeout for {self.lottery_type}")
      raise ModelError("Prediction timeout exceeded")
    except Exception as e:
      logger.error(f"RF prediction error for {self.lottery_type}: {e}")
      raise ModelError(f"Prediction failed: {e}")

  def predict_next_combination(self, last_draw_numbers_f1: List[int], last_draw_numbers_f2: List[int],
                               df_history: Optional[pd.DataFrame] = None) -> Tuple[Optional[List[int]], Optional[List[int]]]:
    """
    Универсальное предсказание следующей комбинации.
    """
    if not self.is_trained:
      logger.warning(f"RF model for {self.lottery_type} is not trained")
      raise ModelNotTrainedError("Model is not trained")

    # Валидация входных данных
    self._validate_input_numbers(last_draw_numbers_f1, last_draw_numbers_f2)

    # Получаем ожидаемое количество признаков
    expected_features = self._get_expected_features_count()
    ...
    if df_history is None or len(df_history) < 5:
      ...
      try:
        # Собираем исторические данные
        history_data = []
        for i in range(min(5, len(df_history))):
          row = df_history.iloc[i]
          f1 = row.get('Числа_Поле1_list', [])
          f2 = row.get('Числа_Поле2_list', [])
          if isinstance(f1, list) and isinstance(f2, list):
            history_data.append((f1, f2))

        # Создаем вектор признаков
        features_array = self._create_feature_vector(
          last_draw_numbers_f1,
          last_draw_numbers_f2,
          history_data,
          df_history
        )
        ...
        print(f"AI Model (RF): Ошибка подготовки признаков: {e}")
        return None, None
  ...
  async def score_combination_async(self, combination_f1: List[int], combination_f2: List[int], 
                                   df_history: pd.DataFrame) -> float:
    """
    Асинхронная оценка предложенной комбинации.
    """
    try:
      loop = asyncio.get_event_loop()
      with concurrent.futures.ThreadPoolExecutor(max_workers=PERF_CONFIG.max_workers) as executor:
        result = await asyncio.wait_for(
          loop.run_in_executor(
            executor,
            partial(self.score_combination, combination_f1, combination_f2, df_history)
          ),
          timeout=PERF_CONFIG.async_timeout
        )
      return result
    except asyncio.TimeoutError:
      logger.error(f"RF scoring timeout for {self.lottery_type}")
      return -float('inf')
    except Exception as e:
      logger.error(f"RF scoring error for {self.lottery_type}: {e}")
      return -float('inf')

  def score_combination(self, combination_f1: List[int], combination_f2: List[int], df_history: pd.DataFrame) -> float:
    """
    ...
    try:
      ...
      if df_history is None or df_history.empty:
        raise InvalidInputError("History data is required")

      # Валидация комбинаций
      self._validate_combination(combination_f1, combination_f2)
      ...
      logger.debug(f"Lottery: {self.lottery_type}, Field sizes: {self.field1_size}x{self.field2_size}")
      logger.debug(f"Last draw data: f1={last_f1} (type: {type(last_f1)}), f2={last_f2} (type: {type(last_f2)})")
      ...
      if not (isinstance(last_f1, list) and len(last_f1) == self.field1_size and
        ...
        # Попытка восстановить данные из других колонок
        if 'Числа_Поле1' in last_draw and 'Числа_Поле2' in last_draw:
          try:
            if isinstance(last_draw['Числа_Поле1'], str):
              last_f1 = [int(x.strip()) for x in last_draw['Числа_Поле1'].split(',')]
            if isinstance(last_draw['Числа_Поле2'], str):
              last_f2 = [int(x.strip()) for x in last_draw['Числа_Поле2'].split(',')]
            logger.info(f"Восстановлены данные: f1={last_f1}, f2={last_f2}")
          except Exception as e:
            logger.error(f"Failed to parse string data: {e}")
            
        if not (isinstance(last_f1, list) and len(last_f1) == self.field1_size and
                isinstance(last_f2, list) and len(last_f2) == self.field2_size):
          error_msg = f"Invalid last draw data for {self.lottery_type}: f1={last_f1} (expected list of {self.field1_size}), f2={last_f2} (expected list of {self.field2_size})"
      ...
      try:
        ...
        logger.error(f"Error creating features for scoring: {e}")
      ...
      for i in range(self.field1_size):
        ...
        if hasattr(model_pos, 'predict_proba') and classes_pos.size > 0:
          ...
          raise ModelNotTrainedError(f"Field1 position {i} model not trained")
      ...
      for i in range(self.field2_size):
        ...
        if hasattr(model_pos, 'predict_proba') and classes_pos.size > 0:
          ...
          raise ModelNotTrainedError(f"Field2 position {i} model not trained")
      ...
      logger.error(f"Unexpected error in score_combination: {e}")
      return -float('inf')

  def _validate_input_numbers(self, f1: List[int], f2: List[int]) -> None:
    """Валидация входных чисел для предсказания"""
    if not (isinstance(f1, list) and len(f1) == self.field1_size and
            isinstance(f2, list) and len(f2) == self.field2_size):
      raise InvalidInputError(f"Expected {self.field1_size} and {self.field2_size} numbers")
    
    if not all(1 <= n <= self.field1_max for n in f1):
      raise InvalidInputError(f"Field1 numbers must be in range 1-{self.field1_max}")
    if not all(1 <= n <= self.field2_max for n in f2):
      raise InvalidInputError(f"Field2 numbers must be in range 1-{self.field2_max}")

  def _validate_combination(self, f1: List[int], f2: List[int]) -> None:
    """Валидация комбинации для оценки"""
    if (not isinstance(f1, list) or len(f1) != self.field1_size or
        not isinstance(f2, list) or len(f2) != self.field2_size):
      raise InvalidInputError(f"Invalid combination sizes")
    
    if not all(1 <= n <= self.field1_max for n in f1):
      raise InvalidInputError(f"Field1 combination out of range")
    if not all(1 <= n <= self.field2_max for n in f2):
      raise InvalidInputError(f"Field2 combination out of range")

  def _get_expected_features_count(self) -> Optional[int]:
    """Получает ожидаемое количество признаков модели"""
    try:
      if hasattr(self.models_f1[0], 'n_features_in_'):
        return self.models_f1[0].n_features_in_
      elif hasattr(self.models_f1[0], 'n_features_'):
        return self.models_f1[0].n_features_
    except (AttributeError, TypeError, IndexError):
      pass
    return self._feature_vector_length if self._feature_vector_length > 0 else None
...
class LotteryLSTMOps:
  ...
  def _prepare_sequences(self, df_history: pd.DataFrame) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
    """
  ...
  def train(self, df_history: pd.DataFrame, epochs=10, batch_size=8, learning_rate=0.001):
    """
  ...
  async def train_async(self, df_history: pd.DataFrame, epochs=10, batch_size=8, learning_rate=0.001):
    """
    Асинхронная обертка для универсального обучения LSTM модели.
    """
    loop = asyncio.get_event_loop()
    # Используем ThreadPoolExecutor для потенциально блокирующей задачи
    with concurrent.futures.ThreadPoolExecutor() as executor:
      result = await loop.run_in_executor(
        executor,
        partial(self._train_sync_wrapper, df_history, epochs, batch_size, learning_rate)
      )
    return result

  def _train_sync_wrapper(self, df_history, epochs, batch_size, learning_rate):
    """Синхронная обертка для вызова основного метода обучения."""
    self.train(df_history, epochs, batch_size, learning_rate)
    return self.is_trained

  def predict_next_combination(self, df_last_n_draws: pd.DataFrame) -> Tuple[List[int], List[int]]:
    """
...
class ModelManager:
  ...
  def get_rf_model(self, lottery_type: str, lottery_config: dict) -> RFModel:
    ...
    with self._lock:
      ...
      self._rf_models[lottery_type] = RFModel(lottery_config=lottery_config)
  ...
  async def train_all_models_async(self, lottery_configs: dict, data_fetcher_func):
    """
    Асинхронно обучает все модели для всех лотерей.
    """
    tasks = []
    results = {}

    for lottery_type, config in lottery_configs.items():
      task = asyncio.create_task(self._train_lottery_models_async(lottery_type, config, data_fetcher_func))
    ...
    for lottery_type, task in tasks:
      try:
        ...
        print(f"Model Manager: Ошибка при асинхронном обучении моделей для {lottery_type}: {e}")
        results[lottery_type] = {'rf_trained': False, 'lstm_trained': False, 'error': str(e)}

    return results

  async def _train_lottery_models_async(self, lottery_type: str, config: dict, data_fetcher_func):
    """
    Асинхронно обучает модели для одной лотереи.
    """
    print(f"Model Manager: Начало асинхронного обучения моделей для {lottery_type}")
    try:
      # Получаем данные для этой лотереи
      from backend.app.core.lottery_context import LotteryContext
      with LotteryContext(lottery_type):
      ...
      if not df.empty:
        ...
        # Создаем задачи для асинхронного обучения
        rf_train_task = rf_model.train_async(df)
        lstm_train_task = lstm_model.train_async(df)

        # Выполняем обучение параллельно
        rf_trained, lstm_trained = await asyncio.gather(rf_train_task, lstm_train_task, return_exceptions=True)

        # Обрабатываем возможные исключения
        if isinstance(rf_trained, Exception):
          print(f"Model Manager: Ошибка RF обучения для {lottery_type}: {rf_trained}")
          rf_result = False
        else:
          rf_result = rf_trained

        if isinstance(lstm_trained, Exception):
          print(f"Model Manager: Ошибка LSTM обучения для {lottery_type}: {lstm_trained}")
          lstm_result = False
        else:
          lstm_result = lstm_trained

        print(f"Model Manager: Модели для {lottery_type} обучены асинхронно успешно")
        return {
          'rf_trained': rf_result,
          'lstm_trained': lstm_result,
          'error': None
        }
      else:
        print(f"Model Manager: Нет данных для асинхронного обучения {lottery_type}")
        return {
          'rf_trained': False,
          'lstm_trained': False,
  ...
  def get_model_status(self) -> dict:
    ...
    status = {}
    with self._lock:
      for lottery_type in set(list(self._rf_models.keys()) + list(self._lstm_models.keys())):
        rf_model = self._rf_models.get(lottery_type)
  ...
  def clear_lottery_models(self, lottery_type: str):
    """Очищает модели для конкретной лотереи (при изменении конфигурации)"""
    with self._lock:
      if lottery_type in self._rf_models:
        print(f"[CLEAR] Удаление RF модели для {lottery_type}")
        del self._rf_models[lottery_type]
      if lottery_type in self._lstm_models:
        print(f"[CLEAR] Удаление LSTM модели для {lottery_type}")
        del self._lstm_models[lottery_type]
      print(f"[CLEAR] Модели для {lottery_type} очищены")
...
class GlobalModelProxy:
  ...
  def _get_cached_model(self):
    ...
    print(f"[DEBUG] _get_cached_model: текущая лотерея = {current_lottery}")
    print(f"[DEBUG] _get_cached_model: кэшированная лотерея = {self._cached_lottery}")
    print(f"[DEBUG] _get_cached_model: модель в кэше = {self._cached_model is not None}")
    ...
    if self._cached_lottery != current_lottery or self._cached_model is None:
      print(f"[UPDATE] Получение RF модели для {current_lottery}")
      ...
      print(f"[DEBUG] Полученная модель обучена: {model.is_trained if model else 'None'}")
      self._cached_model = model
      self._cached_lottery = current_lottery
    else:
      print(f"[OK] Используем кэшированную модель для {current_lottery}")
  ...
  def train(self, df_history):
    ...
    if model:
      ...
      print("[CACHE_CLEAR] RF кэш очищен после переобучения")
      return model.train(df_history)
    return False

  async def train_async(self, df_history):
    """Асинхронная версия метода train."""
    model = self._get_cached_model()
    if model:
      # Очищаем кэш после переобучения
      from backend.app.core.rf_cache import GLOBAL_RF_CACHE
      ...
      # Вызываем асинхронный метод обучения
      return await model.train_async(df_history)
    return False
  ...
  def score_combination(self, f1, f2, df_history):
    ...
    print(f"[SCORE_DEBUG] Вызов score_combination для лотереи: {data_manager.CURRENT_LOTTERY}")
    print(f"[SCORE_DEBUG] Комбинация: f1={f1}, f2={f2}")
    ...
    if not model or not model.is_trained:
      print(f"[ERROR] score_combination: Модель не готова (model={model}, trained={model.is_trained if model else 'None'})")
      return -float('inf')
    try:
      ...
      print(f"[ERROR] Ошибка оценки комбинации: {e}")
      return -float('inf')
...
class AutoUpdateScheduler:
  ...
  async def start_scheduler(self):
    ...
    self.logger.info(f"[OK] Планировщик активен для {len(self.schedules)} лотерей")
  ...
  async def stop_scheduler(self):
    ...
    self.tasks.clear()
    self.logger.info("[OK] Планировщик полностью остановлен")
  ...
  async def _schedule_lottery_updates(self, lottery_type: str):
    ...
    while self.is_running:
      try:
        ...
        self.logger.error(f"[ERROR] Ошибка в планировщике {lottery_type}: {e}")
        # Ждем 1 минуту перед повторной попыткой
        await asyncio.sleep(60)

  async def _update_and_retrain_lottery(self, lottery_type: str):
    """Обновление данных и асинхронное переобучение модели для конкретной лотереи"""
    try:
      self.logger.info(f"[UPDATE] Проверка новых тиражей для {lottery_type}...")
      # Импортируем модули
      ...
      with LotteryContext(lottery_type):
        ...
        if new_count > old_count:
          ...
          # Создаем задачи для асинхронного обучения
          rf_train_task = rf_model.train_async(new_df)
          lstm_train_task = lstm_model.train_async(new_df)

          # Выполняем обучение параллельно
          try:
            self.logger.info(f"🧠 Асинхронное переобучение моделей для {lottery_type}...")
            rf_result, lstm_result = await asyncio.gather(rf_train_task, lstm_train_task)

            # Проверяем результаты
            rf_trained = rf_result if not isinstance(rf_result, Exception) else False
            lstm_trained = lstm_result if not isinstance(lstm_result, Exception) else False

            status = "[OK]" if rf_trained else "[ERROR]"
            lstm_status = "[OK]" if lstm_trained else "⚠️"
            self.logger.info(f"🎉 {lottery_type}: модели обновлены асинхронно! RF={status}, LSTM={lstm_status}")
          except Exception as e:
            self.logger.error(f"💥 Ошибка асинхронного переобучения {lottery_type}: {e}")
            # Можно добавить fallback на синхронное обучение при необходимости
        else:
